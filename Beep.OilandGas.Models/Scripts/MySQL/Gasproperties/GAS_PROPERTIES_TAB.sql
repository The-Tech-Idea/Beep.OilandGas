CREATE TABLE `GAS_PROPERTIES`
(
    `GAS_PROPERTIES_ID` VARCHAR(40) NOT NULL,
    `GAS_COMPOSITION_ID` VARCHAR(40),
    `PRESSURE` DECIMAL(18,2),
    `TEMPERATURE` DECIMAL(18,2),
    `Z_FACTOR` DECIMAL(18,2),
    `VISCOSITY` DECIMAL(18,2),
    `DENSITY` DECIMAL(18,2),
    `SPECIFIC_GRAVITY` DECIMAL(18,2),
    `MOLECULAR_WEIGHT` DECIMAL(18,2),
    `PSEUDO_REDUCED_PRESSURE` DECIMAL(18,2),
    `PSEUDO_REDUCED_TEMPERATURE` DECIMAL(18,2),
    `PSEUDO_CRITICAL_PRESSURE` DECIMAL(18,2),
    `PSEUDO_CRITICAL_TEMPERATURE` DECIMAL(18,2),
    `ACTIVE_IND` VARCHAR(1) DEFAULT 'Y',
    `PPDM_GUID` VARCHAR(40),
    `REMARK` VARCHAR(2000),
    `SOURCE` VARCHAR(40),
    `ROW_CREATED_DATE` DATETIME,
    `ROW_CREATED_BY` VARCHAR(30),
    `ROW_CHANGED_DATE` DATETIME,
    `ROW_CHANGED_BY` VARCHAR(30),
    `ROW_EFFECTIVE_DATE` DATETIME,
    `ROW_EXPIRY_DATE` DATETIME,
    `ROW_QUALITY` VARCHAR(40),
    `AREA_ID` VARCHAR(40),
    `AREA_TYPE` VARCHAR(50),
    `BUSINESS_ASSOCIATE_ID` VARCHAR(40),
    `EFFECTIVE_DATE` DATETIME,
    `EXPIRY_DATE` DATETIME
);
