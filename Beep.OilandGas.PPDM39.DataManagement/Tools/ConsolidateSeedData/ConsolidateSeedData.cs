...
namespace Beep.OilandGas.PPDM39.DataManagement.Tools.ConsolidateSeedData
{
    public class ConsolidateSeedData
    {
        public static void Main(string[] args)
        {
            ...
            // Go up from bin/Debug/netX.0 to project root
            while (!Directory.Exists(Path.Combine(baseDir, "Core")) && baseDir != Path.GetDirectoryName(baseDir))
            {
                baseDir = Path.GetDirectoryName(baseDir);
            }
            
            var csvDataPath = Path.Combine(baseDir, "Core", "SeedData", "PPDMCSVData.json");
            var referenceDataPath = Path.Combine(baseDir, "SeedData", "Templates", "PPDMReferenceData.json");
            
            if (!File.Exists(csvDataPath))
            {
                Console.WriteLine($"CSV data file not found: {csvDataPath}");
                return;
            }
            
            if (!File.Exists(referenceDataPath))
            {
                Console.WriteLine($"Reference data file not found: {referenceDataPath}");
                return;
            }
