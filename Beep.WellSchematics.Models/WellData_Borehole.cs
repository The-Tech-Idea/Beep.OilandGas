...
namespace Beep.WellSchematics
{
    public class WellData_Borehole
    {
        ...
        public Dictionary<int, List<(float x, float y)>> outerWellborePoints { get; set; } = new Dictionary<int, List<(float x, float y)>>();
        public Dictionary<int, List<(float x, float y)>> innerWellborePoints { get; set; } = new Dictionary<int, List<(float x, float y)>>();

        public Dictionary<int, List<(float inclination, float depth)>> inclinations { get; set; } = new Dictionary<int, List<(float inclination, float depth)>>();
        public Dictionary<int, List<float>> boreholeDepths { get; set; } = new Dictionary<int, List<float>>();
        public Dictionary<int, List<float>> boreholeCurvatures { get; set; } = new Dictionary<int, List<float>>();
