...
using Beep.OilandGas.Accounting.Operational.Inventory;
using Beep.OilandGas.Accounting.Operational.Allocation;
using Beep.OilandGas.Accounting.Operational.Royalty;
            ...
            List<SalesTransaction> transactions)
        {
            ...
            if (periodInventories.Count > 0)
            {
                report.Inventory = new InventoryReportSummary
                {
                    ...
                    Deliveries = periodTickets.Where(t => t.DispositionType == Operational.Production.DispositionType.Sale)
                        .Sum(t => t.NetVolume)
                };
            }
            ...
            var report = new GovernmentalReport
            {
                ...
                Taxes = new GovernmentalTaxData
                {
                    SeveranceTax = periodTransactions.Sum(t => 
                        t.Taxes.Where(tax => tax.TaxType == Revenue.ProductionTaxType.Severance)
                        .Sum(tax => tax.Amount)),
                    AdValoremTax = periodTransactions.Sum(t => 
                        t.Taxes.Where(tax => tax.TaxType == Revenue.ProductionTaxType.AdValorem)
                        .Sum(tax => tax.Amount))
                }
            };
