using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using Microsoft.Extensions.Logging;
using TheTechIdea.Beep.Editor;
using Beep.OilandGas.Accounting.Constants;
using Beep.OilandGas.Models.Data.ProductionAccounting;
using Beep.OilandGas.PPDM39.Repositories;
using Beep.OilandGas.PPDM39.Core.Metadata;
using Beep.OilandGas.PPDM39.DataManagement.Core;

namespace Beep.OilandGas.Accounting.Services
{
    /// <summary>
    /// IAS 19 employee benefits: accruals and settlements.
    /// </summary>
    public class EmployeeBenefitsService
    {
        private readonly IDMEEditor _editor;
        private readonly ICommonColumnHandler _commonColumnHandler;
        private readonly IPPDM39DefaultsRepository _defaults;
        private readonly IPPDMMetadataRepository _metadata;
        private readonly AccountingBasisPostingService _basisPosting;
        private readonly IAccountMappingService? _accountMapping;
        private readonly ILogger<EmployeeBenefitsService> _logger;
        private const string ConnectionName = "PPDM39";

        public EmployeeBenefitsService(
            IDMEEditor editor,
            ICommonColumnHandler commonColumnHandler,
            IPPDM39DefaultsRepository defaults,
            IPPDMMetadataRepository metadata,
            AccountingBasisPostingService basisPosting,
            ILogger<EmployeeBenefitsService> logger,
            IAccountMappingService? accountMapping = null)
        {
            _editor = editor ?? throw new ArgumentNullException(nameof(editor));
            _commonColumnHandler = commonColumnHandler ?? throw new ArgumentNullException(nameof(commonColumnHandler));
            _defaults = defaults ?? throw new ArgumentNullException(nameof(defaults));
            _metadata = metadata ?? throw new ArgumentNullException(nameof(metadata));
            _basisPosting = basisPosting ?? throw new ArgumentNullException(nameof(basisPosting));
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
            _accountMapping = accountMapping;
        }

        public async Task<ACCOUNTING_COST> RecordBenefitAccrualAsync(
            string benefitPlanName,
            DateTime accrualDate,
            decimal amount,
            string userId,
            string? benefitPlanId = null,
            string? connectionName = null)
        {
            if (string.IsNullOrWhiteSpace(benefitPlanName))
                throw new ArgumentNullException(nameof(benefitPlanName));
            if (amount <= 0m)
                throw new InvalidOperationException("Benefit accrual amount must be positive");
            if (string.IsNullOrWhiteSpace(userId))
                throw new ArgumentNullException(nameof(userId));

            var cn = connectionName ?? ConnectionName;
            var cost = new ACCOUNTING_COST
            {
                ACCOUNTING_COST_ID = Guid.NewGuid().ToString(),
                FINANCE_ID = benefitPlanId ?? "EMPLOYEE_BENEFIT",
                COST_TYPE = "EMPLOYEE_BENEFIT",
                COST_CATEGORY = "ACCRUAL",
                AMOUNT = amount,
                COST_DATE = accrualDate,
                IS_CAPITALIZED = _defaults.GetActiveIndicatorNo(),
                IS_EXPENSED = _defaults.GetActiveIndicatorYes(),
                DESCRIPTION = $"Benefit accrual: {benefitPlanName}",
                SOURCE = "IAS19",
                ACTIVE_IND = _defaults.GetActiveIndicatorYes(),
                PPDM_GUID = Guid.NewGuid().ToString(),
                ROW_CREATED_BY = userId,
                ROW_CREATED_DATE = DateTime.UtcNow
            };

            var repo = await GetRepoAsync<ACCOUNTING_COST>("ACCOUNTING_COST", cn);
            await repo.InsertAsync(cost, userId);

            await _basisPosting.PostBalancedEntryByAccountAsync(
                GetAccountId(AccountMappingKeys.EmployeeBenefitExpense, DefaultGlAccounts.EmployeeBenefitExpense),
                GetAccountId(AccountMappingKeys.EmployeeBenefitLiability, DefaultGlAccounts.EmployeeBenefitLiability),
                amount,
                $"Employee benefit accrual {benefitPlanName}",
                userId,
                cn);

            _logger?.LogInformation("Recorded IAS 19 benefit accrual {Plan} amount {Amount}",
                benefitPlanName, amount);

            return cost;
        }

        public async Task<ACCOUNTING_COST> RecordBenefitPaymentAsync(
            string benefitPlanName,
            DateTime paymentDate,
            decimal amount,
            string userId,
            string? benefitPlanId = null,
            string? cashAccountId = null,
            string? connectionName = null)
        {
            if (string.IsNullOrWhiteSpace(benefitPlanName))
                throw new ArgumentNullException(nameof(benefitPlanName));
            if (amount <= 0m)
                throw new InvalidOperationException("Benefit payment amount must be positive");
            if (string.IsNullOrWhiteSpace(userId))
                throw new ArgumentNullException(nameof(userId));

            var cn = connectionName ?? ConnectionName;
            var cost = new ACCOUNTING_COST
            {
                ACCOUNTING_COST_ID = Guid.NewGuid().ToString(),
                FINANCE_ID = benefitPlanId ?? "EMPLOYEE_BENEFIT",
                COST_TYPE = "EMPLOYEE_BENEFIT",
                COST_CATEGORY = "PAYMENT",
                AMOUNT = amount,
                COST_DATE = paymentDate,
                IS_CAPITALIZED = _defaults.GetActiveIndicatorNo(),
                IS_EXPENSED = _defaults.GetActiveIndicatorNo(),
                DESCRIPTION = $"Benefit payment: {benefitPlanName}",
                SOURCE = "IAS19",
                ACTIVE_IND = _defaults.GetActiveIndicatorYes(),
                PPDM_GUID = Guid.NewGuid().ToString(),
                ROW_CREATED_BY = userId,
                ROW_CREATED_DATE = DateTime.UtcNow
            };

            var repo = await GetRepoAsync<ACCOUNTING_COST>("ACCOUNTING_COST", cn);
            await repo.InsertAsync(cost, userId);

            var cashAccount = string.IsNullOrWhiteSpace(cashAccountId)
                ? GetAccountId(AccountMappingKeys.Cash, DefaultGlAccounts.Cash)
                : cashAccountId;

            await _basisPosting.PostBalancedEntryByAccountAsync(
                GetAccountId(AccountMappingKeys.EmployeeBenefitLiability, DefaultGlAccounts.EmployeeBenefitLiability),
                cashAccount,
                amount,
                $"Employee benefit payment {benefitPlanName}",
                userId,
                cn);

            _logger?.LogInformation("Recorded IAS 19 benefit payment {Plan} amount {Amount}",
                benefitPlanName, amount);

            return cost;
        }

        private async Task<PPDMGenericRepository> GetRepoAsync<T>(string tableName, string cn)
        {
            var metadata = await _metadata.GetTableMetadataAsync(tableName);
            
            return new PPDMGenericRepository(
                _editor, _commonColumnHandler, _defaults, _metadata,
                typeof(T), cn, tableName);
        }

        private string GetAccountId(string key, string fallback)
        {
            return _accountMapping?.GetAccountId(key) ?? fallback;
        }
    }
}



