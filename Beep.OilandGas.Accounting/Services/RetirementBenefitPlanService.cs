...
{
    /// <summary>
    /// IAS 26 retirement benefit plan transactions and disclosures.
    /// </summary>
    public class RetirementBenefitPlanService
    {
        ...
        private readonly ILogger<RetirementBenefitPlanService> _logger;
        private const string ConnectionName = "PPDM39";

        public RetirementBenefitPlanService(
            IDMEEditor editor,
            ICommonColumnHandler commonColumnHandler,
            IPPDM39DefaultsRepository defaults,
            IPPDMMetadataRepository metadata,
            AccountingBasisPostingService basisPosting,
            ILogger<RetirementBenefitPlanService> logger,
        ...
        public async Task<ACCOUNTING_COST> RecordPlanContributionAsync(
            string planName,
            ...
            string? connectionName = null)
        {
            ...
            var cost = new ACCOUNTING_COST
            {
                ...
                FINANCE_ID = planId ?? "RETIREMENT_PLAN",
                COST_TYPE = "RETIREMENT_PLAN",
                COST_CATEGORY = "CONTRIBUTION",
                AMOUNT = amount,
                COST_DATE = contributionDate,
                IS_CAPITALIZED = _defaults.GetActiveIndicatorNo(),
                IS_EXPENSED = _defaults.GetActiveIndicatorNo(),
                DESCRIPTION = $"Plan contribution: {planName}",
                SOURCE = "IAS26",
            ...
            await _basisPosting.PostBalancedEntryByAccountAsync(
                GetAccountId(AccountMappingKeys.RetirementPlanAsset, DefaultGlAccounts.RetirementPlanAsset),
                cashAccount,
                amount,
                $"Retirement plan contribution {planName}",
                userId,
                cn);

            _logger?.LogInformation("Recorded IAS 26 plan contribution {Plan} amount {Amount}",
                planName, amount);

            return cost;
        }

        public async Task<ACCOUNTING_COST> RecordBenefitPaymentAsync(
            string planName,
            ...
            string? connectionName = null)
        {
            ...
            var cost = new ACCOUNTING_COST
            {
                ...
                FINANCE_ID = planId ?? "RETIREMENT_PLAN",
                COST_TYPE = "RETIREMENT_PLAN",
                COST_CATEGORY = "BENEFIT_PAYMENT",
                AMOUNT = amount,
                COST_DATE = paymentDate,
                IS_CAPITALIZED = _defaults.GetActiveIndicatorNo(),
                IS_EXPENSED = _defaults.GetActiveIndicatorNo(),
                DESCRIPTION = $"Benefit payment: {planName}",
                SOURCE = "IAS26",
                ACTIVE_IND = _defaults.GetActiveIndicatorYes(),
                PPDM_GUID = Guid.NewGuid().ToString(),
                ROW_CREATED_BY = userId,
            ...
            await _basisPosting.PostBalancedEntryByAccountAsync(
                GetAccountId(AccountMappingKeys.EmployeeBenefitLiability, DefaultGlAccounts.EmployeeBenefitLiability),
                cashAccount,
                amount,
                $"Retirement plan benefit payment {planName}",
                userId,
                cn);

            _logger?.LogInformation("Recorded IAS 26 benefit payment {Plan} amount {Amount}",
                planName, amount);
