...
- [ ] If touching `Beep.OilandGas.Web` UI, read `MudBlazor_Docs/README.md` (solution root) and the relevant local MudBlazor component docs before editing `.razor`, layout, theme, navigation, dialogs, steppers, tabs, or data grids.
...
- Before changing Blazor UI structure or component markup, start with `MudBlazor_Docs/README.md` (solution root).
