...
- **Regulatory Authorities**: BLM, USACE, EPA, BOEM, BSEE (see `RegulatoryAuthority` enum)
- **Coverage**: Federal-level permits across multiple states

## JSON form configuration (`Configs/`)

Shipped templates use `jurisdiction_<authority>_<scope>.json` (see `Forms/JurisdictionConfigLoader.cs`). Files present in the repository:

| Config file | Typical authority / scope |
|---------------|---------------------------|
| `jurisdiction_anp.json` | ANP (Brazil) |
| `jurisdiction_aogcc.json` | AOGCC (Alaska) |
| `jurisdiction_arg_mendoza.json` | ARG_MENDOZA |
| `jurisdiction_arg_neuquen.json` | ARG_NEUQUEN |
| `jurisdiction_asea.json` | ASEA (Mexico) |
| `jurisdiction_blm.json` | BLM |
| `jurisdiction_cnh.json` | CNH (Mexico) |
| `jurisdiction_cogcc.json` | COGCC |
| `jurisdiction_cec.json` | CEC (California) |
| `jurisdiction_dpr.json` | DPR (Nigeria) |
| `jurisdiction_epa.json` | EPA |
| `jurisdiction_kz_moe.json` | KZ_MOE |
| `jurisdiction_ladnr.json` | LADNR |
| `jurisdiction_ndic.json` | NDIC |
| `jurisdiction_nmocd.json` | NMOCD |
| `jurisdiction_nopsema.json` | NOPSEMA |
| `jurisdiction_npd.json` | NPD (Norway) |
| `jurisdiction_nsta.json` | NSTA (UK) |
| `jurisdiction_occ.json` | OCC |
| `jurisdiction_qld_dnrme.json` | QLD_DNRME |
| `jurisdiction_ser.json` | SER (Saskatchewan) |
| `jurisdiction_skkmigas.json` | SKKMigas |
| `jurisdiction_usace.json` | USACE |
| `jurisdiction_wa_dmirs.json` | WA_DMIRS |
| `jurisdiction_wogcc.json` | WOGCC |
| `jurisdiction_nldet.json` | NLDET |
| `jurisdiction_sample.json` | Sample / test |

If `RegulatoryAuthority` has a member with no JSON file yet, validation and form payloads still run with defaults until a config is added.
