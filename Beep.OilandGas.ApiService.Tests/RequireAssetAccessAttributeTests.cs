...
public class RequireAssetAccessAttributeTests
{
    [Fact]
    public async Task OnAuthorizationAsync_WhenNameIdentifierDiffersFromIdentity_UsesNameIdentifier()
    {
        ...
        context.RouteData.Values["fieldId"] = "FIELD-1";

        var attribute = new RequireAssetAccessAttribute("fieldId", requiredPermission: "ViewProduction");
        await attribute.OnAuthorizationAsync(context);

        Assert.Null(context.Result);
        Assert.Equal("trusted-user", access.LastUserId);
        Assert.Equal("FIELD-1", access.LastAssetId);
        Assert.Equal("FIELD", access.LastAssetType);
    }

    [Fact]
    public async Task OnAuthorizationAsync_WhenDenied_SetsForbid()
    {
        var access = new TestAccessControlService
        {
        ...
        context.RouteData.Values["wellId"] = "WELL-9";

        var attribute = new RequireAssetAccessAttribute("wellId", requiredPermission: "EditWells");
        await attribute.OnAuthorizationAsync(context);
