...
public static class PPDM39TableMapping
{
    ...
    /// <summary>
    /// Full registry of every known PPDM39 table.
    /// Each entry carries the category ID plus its FK-derived parents and children.
    /// Keyed by upper-case table name.
    /// </summary>
    /// <summary>
    /// Full registry of every known PPDM39 table — hardcoded, no runtime file I/O.
    /// Delegates to <see cref="PPDM39TableRegistry.All"/>.
    /// </summary>
    public static IReadOnlyDictionary<string, PPDM39TableEntry> TableRegistry => PPDM39TableRegistry.All;

    /// <summary>Maps each child table to the unique set of parent tables it references via foreign keys.</summary>
    public static IReadOnlyDictionary<string, IReadOnlyList<string>> TableParents { get; private set; } =
    ...
    public static IReadOnlyDictionary<string, IReadOnlyList<string>> TableChildren { get; private set; } =
        new Dictionary<string, IReadOnlyList<string>>(System.StringComparer.OrdinalIgnoreCase);

    /// <summary>Always true — hierarchy is fully hardcoded.</summary>
    public static bool IsHierarchyLoaded => true;
    ...
    static PPDM39TableMapping()
    {
        ...
        // Derive TableParents and TableChildren directly from the hardcoded registry.
        // No file I/O — PPDM39TableRegistry.All is fully static.
        var parentsMap  = new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase);
        var childrenMap = new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase);

        foreach (var entry in PPDM39TableRegistry.All.Values)
        {
            if (entry.IsRoot && entry.Children.Count > 0)
            {
                var childNames = entry.Children.Select(c => c.TableName).ToList();
                childrenMap[entry.TableName] = childNames;
                var parentList = (IReadOnlyList<string>)new[] { entry.TableName };
                foreach (var child in entry.Children)
                {
                    parentsMap[child.TableName] = parentList;
                    if (entry.CategoryId != 0 && !TableToCategoryMap.ContainsKey(child.TableName))
                        TableToCategoryMap[child.TableName] = entry.CategoryId;
                }
            }

            if (entry.CategoryId != 0 && !TableToCategoryMap.ContainsKey(entry.TableName))
                TableToCategoryMap[entry.TableName] = entry.CategoryId;
        }

        TableParents  = parentsMap;
        TableChildren = childrenMap;
    }
    ...
    /// <summary>Returns the unified <see cref="PPDM39TableEntry"/> for a table, or null if unknown.</summary>
    public static PPDM39TableEntry? GetEntry(string tableName) =>
        !string.IsNullOrWhiteSpace(tableName) && TableRegistry.TryGetValue(tableName, out var entry)
            ? entry
            : null;
    ...
    /// <summary>Returns all tables that have no FK parents (root entities in the schema).</summary>
    public static IReadOnlyList<string> GetRootTables()
    {
        var allTables = new HashSet<string>(TableToCategoryMap.Keys, StringComparer.OrdinalIgnoreCase);
        // also include any tables referenced only in FK metadata but not in the category map
        foreach (var key in TableParents.Keys) allTables.Add(key);
        foreach (var key in TableChildren.Keys) allTables.Add(key);
        return allTables.Where(t => !TableParents.ContainsKey(t)).OrderBy(t => t).ToList();
    }

    /// <summary>Recursively collects all ancestor tables (parents, grandparents, …) for a given table.</summary>
    public static IReadOnlyList<string> GetAncestors(string tableName)
    {
        var visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        CollectAncestors(tableName, visited);
        visited.Remove(tableName);
        return visited.OrderBy(x => x).ToList();
    }

    /// <summary>Recursively collects all descendant tables (children, grandchildren, …) for a given table.</summary>
    public static IReadOnlyList<string> GetDescendants(string tableName)
    {
        var visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        CollectDescendants(tableName, visited);
        visited.Remove(tableName);
        return visited.OrderBy(x => x).ToList();
    }

    /// <summary>Returns the depth of a table in the hierarchy (root = 0). Returns -1 if unknown.</summary>
    public static int GetDepth(string tableName)
    {
        if (string.IsNullOrWhiteSpace(tableName)) return -1;
        int depth = 0;
        var visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { tableName };
        var current = tableName;
        while (TableParents.TryGetValue(current, out var directParents))
        {
            var next = directParents.FirstOrDefault(p => !visited.Contains(p));
            if (next == null) break;
            visited.Add(next);
            current = next;
            depth++;
    ...
    private static void CollectAncestors(string table, HashSet<string> visited)
    {
        if (!visited.Add(table)) return;
        if (!TableParents.TryGetValue(table, out var parents)) return;
        foreach (var parent in parents)
            CollectAncestors(parent, visited);
    }

    private static void CollectDescendants(string table, HashSet<string> visited)
    {
        if (!visited.Add(table)) return;
        if (!TableChildren.TryGetValue(table, out var kids)) return;
