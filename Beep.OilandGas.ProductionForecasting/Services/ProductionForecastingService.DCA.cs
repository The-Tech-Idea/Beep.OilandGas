    ...
    public partial class ProductionForecastingService
    {
        public async Task<DeclineCurveAnalysisDto> PerformDeclineCurveAnalysisAsync(string wellUWI, DateTime startDate, DateTime endDate)
        {
            ...
            var analysis = new DeclineCurveAnalysisDto
            {
                ...
                Status = "OK",
                HistoricalData = new List<ProductionDataPointDto>()
            };

            foreach (var p in pf.ForecastPoints.Take(10))
            {
                analysis.HistoricalData.Add(new ProductionDataPointDto
                {
        ...
        public async Task<ProductionForecastResultDto> GenerateDCAForecastAsync(string wellUWI, string declineType, DateTime startDate, DateTime endDate, int forecastPeriod)
        {
            ...
            Beep.OilandGas.Models.ProductionForecasting.ProductionForecast pf;
            ...
            var dto = new ProductionForecastResultDto
            {
            ...
            foreach (var p in pf.ForecastPoints)
            {
                ...
                dto.ForecastPoints.Add(new ProductionForecastPointDto
                {
                    Date = date,
                    OilRate = p.ProductionRate,
        ...
        public async Task<ProbabilisticForecastDto> GenerateProbabilisticForecastAsync(string wellUWI, string declineType, int forecastPeriod, int iterations = 1000)
        {
            var baseForecast = await GenerateForecastAsync(wellUWI, null, declineType, forecastPeriod);
            var probabilistic = new ProbabilisticForecastDto
            {
                ...
                RiskAnalysis = new RiskAnalysisResultDto()
            };
            await Task.CompletedTask;
