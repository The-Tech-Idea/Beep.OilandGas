# Engineering/Calculation Projects — Comprehensive Scan Report
...
| Component | Location |
|-----------|----------|
| `WellTestAnalyzer` | `WellTestAnalyzer.cs` (entry point) |
...
| `WellTestConstants` | `Constants/WellTestConstants.cs` |
...
| Value | File | Line | Issue |
|-------|------|------|-------|
| `"C"`, `"D"`, `"HL"`, `"HY"`, `"AUTO"` (API rod grades) | `Calculations/RodStringOptimization.cs` | 85, 94, 101, 111 | Magic strings — should be in a `RodGrade` constant class |
...
| `490` (steel density, lb/ft³) | `README.md` | 85 | Magic number — should be in `SuckerRodConstants` |
...
|---------|--------|-------|
| **EnhancedRecovery** | Create `EorMethodConstants` class with all EOR type strings (`WATER_FLOOD`, `GAS_INJECTION`, `CO2_MISCIBLE`, `POLYMER`, `CHEMICAL`, `STEAM`, `THERMAL`, `SURFACTANT`, `ALKALI`) | `Services/EnhancedRecoveryService.cs:49-65`, `Services/EnhancedRecoveryService.Advanced.cs:714-756` |
| **EnhancedRecovery** | Centralize recovery factor defaults (20, 25, 18, 22, 30, 15) and chemical economics parameters | `Services/EnhancedRecoveryService.cs:46-66`, `Services/EnhancedRecoveryService.Advanced.cs:714-742` |
...
| **SuckerRodPumping** | Create `RodGrade` constants class (`C`, `D`, `HL`, `HY`, `AUTO`) | `Calculations/RodStringOptimization.cs:85-116` |
...
| **PipelineAnalysis** | Create `FlowRegime` and `ExportFormat` constants classes | `Services/PipelineAnalysisService.Advanced.cs:30,70`, `Services/PipelineAnalysisService.Reporting.cs:176,194` |
| **HydraulicPumps** | Create `PumpStatus` and `PumpType` constants classes | `Services/HydraulicPumpService.Advanced.cs:42,47,60,73` |
| **OilProperties** | Create `CorrelationMethod` constants class | `Services/OilPropertiesService.cs:160`, `Services/OilPropertiesService.Advanced.cs:36` |
